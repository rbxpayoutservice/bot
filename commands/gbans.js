            ...
            try {
                ...
                if(!bans.length) {
                    ...
                    let details = `
                        ...
                        ID: \`${ban.id}\`
                        Reason: \`${ban.reason}\`
                        Banned By: \`${(await client.robloxInterface.getById(ban.admin)).Username} (${ban.admin})\`
                        Banned At: \`${new Date(ban.at).toLocaleString()}\`
                        Duration: \`${ban.banType === 'Perm' ? ban.banType : `${Math.ceil((ban.unixEndsAt - ban.unixAt) / 86400)} Days`}\`
                    `
                    ...
                    embed.setFooter(`Ban ${page} of ${bans.length}`, message.author.displayAvatarURL);
                        ...
                        .then(msg => {
                            msg.react('⬅️')
                                .then(() => {
                                    ...
                                    const back = msg.createReactionCollector(backFilter, { time: 120000 });
                ...
                message.channel.send(buildEmbed('Error!', ':x: Failed to get GBans for user', null, 'error', message.author));
            }
        })
        .catch((err) => {
