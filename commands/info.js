	...
	getUserOrId(args[0], client, message)
        .then(async ({ userName, userId }) => {
            try {
                ...
                embed.addField("Is Banned: (From Roblox)", String(userInfo.isBanned).charAt(0).toUpperCase() + String(userInfo.isBanned).slice(1));
                ...
                message.channel.send(buildEmbed('Error!', ':x: Failed to get info on user', null, 'error', message.author));
