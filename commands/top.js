...
module.exports.run = async (client, message) => {
    try {
            ...
            .then(msg => {
                msg.react('⬅️')
                    .then(() => {
                        ...
                        const forward = msg.createReactionCollector(forwardFilter, { time: 4800000 });
                        ...
                        back.on('collect', async (r) => {
                            ...
                            console.log(page);
                            const holders = await client.fruitiesInterface.getPointLeaders(5, page);
        ...
        message.channel.send(buildEmbed('Error!', ':x: Unable to get point leaders, please try again later', null, 'error', message.author))
    ...
    function buildPageEmbed(page, holders) {
        return new Promise(async (resolve, reject) => {
            try {
                const usersToFetch = holders.docs.map(holder => client.robloxInterface.getById(holder.robloxId));
                ...
                .then(res => {
                    ...
                    res.forEach((user, index) => {
                        const holder = holders.docs.find(holder => holder.robloxId === user.Id)
                        holderText += `${index+1}: [${user.Username}](https://www.roblox.com/users/${user.Id}/profile) - ${holder.points} Points\n\n`
                    });

                    resolve(buildEmbed('Workerpoint Leaderboard', holderText, null, null, `Page ${page}/${holders.totalPages}`))
                })
                .catch(err => {
                    reject(err);
                })
