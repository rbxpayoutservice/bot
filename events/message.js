...
module.exports = async (client, message) => {
	...
	if(!commandFile) {
		const alias = client.commands.find(cmd => cmd.config.aliases && cmd.config.aliases.includes(command));
				...
				try {
					...
					if (response.status === 'ok' && response.primaryAccount) {
						...
						try {
							...
							message.channel.send(':x: Something unexpected happened and we were unable to verify your rank in the group');
					...
					if (client.userCache[message.author.id].groupRank >= commandFile.config.minimumRank) {
						commandFile.run(client, message, args);
					} else {
						message.channel.send(`:x: You need to be at least rank \`${client.roles.find(role => role.rank === commandFile.config.minimumRank).name}\` to execute this command.`)
					}
				} catch (err) {
					message.channel.send(':x: Failed to check your verification status');
					client.logger.error('Failed to check verification status for ' + message.author.id);
					console.log(err)
